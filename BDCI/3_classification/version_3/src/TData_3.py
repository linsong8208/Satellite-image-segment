...
class TData:
    ...
    def __init__(self, image_size, batch_size, stripe, image_file, sketch_file, area_file):
        self._batch_size = batch_size
        self._image_size = image_size
        self._stripe = stripe

        # 原始数据
        self.image_data = np.copy(np.asarray(Image.open(image_file)))
        self.sketch_data = np.copy(np.asarray(Image.open(sketch_file)))
        self.area_data = np.copy(np.asarray(Image.open(area_file)))

        self.origin_x, self.origin_y = len(self.image_data), len(self.image_data[0])

        self.origin_image_data = np.zeros(shape=[self.origin_x, self.origin_y, 5], dtype=np.uint8)
        self.origin_image_data[:, :, 0: 3] = self.image_data
        self.origin_image_data[:, :, -2] = self.area_data
        self.origin_image_data[:, :, -1] = self.sketch_data

        # 补边之后的数据
        self.new_image_data = np.zeros([self.origin_x + self._image_size - 1,
                                        self.origin_y + self._image_size - 1, 5], dtype=np.uint8)
...
if __name__ == '__main__':
    test_data = TData(227, batch_size=256, stripe=100, image_file="../data/CCF-testing/1.png",
                      sketch_file="../data/sketch_area/1.0_1.7/1_sketch.bmp",
                      area_file="../data/sketch_area/1.0_1.7/1_sketch_area.bmp")
